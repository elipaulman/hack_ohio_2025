class StepDetector {
  ...
  hasEnoughVariance() {
    ...
    const mean =
      this.recentAccelerations.reduce((a, b) => a + b, 0) /
      this.recentAccelerations.length;
    const variance =
      this.recentAccelerations.reduce((sum, val) => {
  ...
  updateAdaptiveThreshold(magnitude) {
    ...
    if (this.magnitudeHistory.length > 10) {
      const avgMagnitude =
        this.magnitudeHistory.reduce((a, b) => a + b, 0) /
  ...
  finishCalibration() {
    ...
    if (this.calibrationMagnitudes.length > 0) {
      ...
      const avgMagnitude =
        this.calibrationMagnitudes.reduce((a, b) => a + b, 0) /
        this.calibrationMagnitudes.length;
    ...
    return {
      ...
      avgMagnitude:
        this.calibrationMagnitudes.length > 0
          ? this.calibrationMagnitudes.reduce((a, b) => a + b, 0) /
            this.calibrationMagnitudes.length
          : 0,
